...
| 欄位 | 預設 | 什麼時候調 |
|---|---|---|
| `LAMBERT_MAX_REVS` | `0` | 最後一棒 Lambert 要考慮的**最大圈數**。`0` = 只看「不繞滿一圈就直接過去」的轉移。`T_max` 是 A 的四個週期，多圈轉移完全合法而且可能便宜非常多——實測官方範例題目 `M=1` 比 `M=0` **省 38%**（267 vs 430 m/s），代價是抵達時間拉長。**時間預算寬鬆時一定要試 `2`。** |
