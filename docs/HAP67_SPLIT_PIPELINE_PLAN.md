...
完整調查見 [HAP67_SPLIT_AWARE_INVESTIGATION.md](HAP67_SPLIT_AWARE_INVESTIGATION.md)。摘要：
- 14 個場景（10 隨機 + 4 往理論硬逼的構造）**0/14** 出現「最佳路線用 >cap 中間棒」。大機動不是
  被**節線攔截**閃掉（→99.99），就是落**終端棒**（→終端拆分器已處理）。
- 理論：攔截問題裡「中間」與「大」**反相關**——有餘裕就用節線逃生（不需大機動）、夾緊才逼出大
  機動但它必落終端。對比 bi-elliptic／三燒換面（兩端固定的軌道轉移）才把工作放中間。
- **決定**：不做中間棒拆分器；`SPLIT_AWARE_SEARCH` 維持預設關但**留著休眠別刪**；**下一輪雙曲線 A
  真題出來重驗**（那才可能是它有用的戰場）。
- 保留：§5 是機制論證非形式證明；構造失敗有一部分是「放鬆時間反而開了節線逃生門」的自拆台。
