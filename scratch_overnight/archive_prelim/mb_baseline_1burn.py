...
config = {
    ...
    "optimization": {"MAX_BURNS": [1], "MAXITER": 300, "POPSIZE": 15, "NUM_THREADS": -1,
...
t0 = time.time()
result = opt._optimize_burn_case(1, scalar_params, vector_params, progress_queue=None)
elapsed = time.time() - t0
_, best_x, best_score, epochs_run, note = result
print(f"[1棒(有種子)] 分數={best_score:.4f}  代數={epochs_run}  耗時={elapsed:.1f}s  note={note}")
print(f"tw={best_x[0]:.1f}s")
