...
- **所有 Lambert 分支都算，取 Δv 最小的**：
  ...
  - **圈數 M（多圈轉移）**：2026-08-28 加。原本寫死 `M=0`，等於只看「不繞滿一圈就直接過去」的轉移——但 `T_max` 是 A 的**四個週期**，多圈轉移本來就在規則允許範圍內。實測官方範例題目：`M=0` 最省的合法單棒要 430.1 m/s，`M=1` 只要 **267.3 m/s（省 38%）**，代價是抵達時間從 6,145s 拉到 11,928s。用 `strategy.LAMBERT_MAX_REVS` 控制（預設 0 = 舊行為）。
...
- 只實作 zonal harmonic（J2/J3/J4，只跟緯度有關），沒有 tesseral/sectoral 項。產生的
  ...
  而犧牲 GMAT 端真實感的刻意取捨。
- ~~只探索單圈 Lambert（`M=0`）~~ **2026-08-28 已解除**：多圈轉移已整合，用 `strategy.LAMBERT_MAX_REVS` 控制（預設 0 = 舊行為）。2026-08-14 那次「評估後決定暫不整合」的結論被官方範例題目推翻——當時只測了兩個情境，官方那題 `M=1` 省 38%。見第 5 節。
