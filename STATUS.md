...
7. **物理直覺的初始種子** 沒做（讓 L-SHADE 從一個粗略 Hohmann-like 猜測開始，而不是純隨機初始化）——早期評估過投入產出比不確定，沒有動手。**2026-08-14 傍晚有了具體、實測過的理由**：`weird_test.json` 的 `MAX_BURNS=1` 案例裡，真的存在一個合法解 (1189.73 m/s)，但藏在只佔 `T_max` 0.0086% 的窄窗口裡，`MAX_BURNS=[1]` 跑過 2000 代都找不到——地形幾乎完全平坦、只有一根極窄的尖峰，純隨機初始化撞不到。用便宜的低維度網格粗掃候選窗口、當種子塞進初始族群，是目前看到最有機會解決這個問題的方向，見「weird_test.json 深度診斷」那節。仍然只在自編的極端壓力測試上驗證過，值不值得投入，等正式測資公布後再評估。
...
14. **`script_generator.py` 的 `View_ShipBChase` 座標系修復還沒 commit**——2026-08-14 傍晚修的 (原本用固定慣性系偏移量，B 軌道半徑跟地球半徑餘裕不夠時鏡頭會跑進地球裡面；改成綁在 ShipB 身上、永遠指向遠離地球方向的 `ObjectReferenced` 座標系)，已經用真實 GMAT 跑過 `practice_scenario.json` 確認腳本正常解析、InterceptSuccess/MissDistance 不受影響，但改動本身還沒 commit，下次接手先看 `git status` 確認還在不在。
