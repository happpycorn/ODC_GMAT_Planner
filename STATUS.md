...
最後更新：2026-08-14 凌晨（8/13 晚間那次 session 的直接延續，同一個晚上沒間斷）。
`improve-optimizer-and-gmat-integration` 分支已經 fast-forward merge 回 `Master` 並
push，分支本身已刪除——**現在直接在 `Master` 上開發**。分支合併後又陸續追加了 tqdm
進度條修復、console 輸出簡化、把 3 個規則數字搬進 config、config 分組整理、
`sweep_burns.py`、固定燃燒繳交腳本、清依賴、`local.gmat_console_path`、`outputs/`
資料夾建立修復、izzo 例外處理、一個發現後又撤銷的危險改動 (RK4 自適應步長) 這幾項，
全部已 commit 且已 push（最新一筆是 `62f8d6c`）。

**⚠️ 這個 session (8/14 凌晨) 的 working tree 還沒 commit**：改了 `main.py` 跟
`src/script_generator.py`（「固定燃燒版本」加一個 fallback 燃燒值來源，見下面新的一節），
還沒問過使用者要不要 commit/push，先老實記錄現狀，下次接手先看 `git status`/`git diff`
確認這兩個檔案的改動還在不在。

**⚠️ 上一次 (8/13) 最重要的發現，這次 session 已經拆解出根因，見下面新的一節**：
「極端軌道」(SMA ~80,000km、ECC ~0.87) 上 Python 預測跟 GMAT 實測差到四五個數量級、
GMAT 求解器收斂不了——上次只測出現象，這次找到了三層原因，其中兩層已經理解/處理，
第三層（長時間傳播下的模型落差）還沒解決，是目前最優先的未解問題。
...
1. **🔴 最優先，已縮小範圍：長時間傳播 (接近 T_max、繞行目標軌道好幾圈) 下，Python 的純 J2 簡化模型跟 GMAT 的 J2~J4 模型會分岔，最壞測過差到 481 公里**（詳見上面「拆解 GMAT 對不上的真正原因」那一節，2026-08-14）。原本以為是「SMA~80,000km/ECC~0.87 這個量級」的問題，已確認**真正的變數是傳播時長/繞行圈數，不是軌道量級本身**——同一個極端軌道，快速方案 (幾百秒) 精準吻合，被迫拉長到接近 T_max (10 天) 的合法方案就對不上。純步長效應已經排除 (只佔 2.1km，不夠解釋 481km)，目前最懷疑是 GMAT 用 `Degree=4,Order=4` (J2~J4) 而 Python 只有純 J2。初賽 A 是圓軌道、T_max 應該不會太誇張，這個問題**大概率不影響初賽**，但排位賽/四強賽未知。下一步可以考慮：(a) 幫 Python 端的力學模型加 J3/J4 看能不能縮小差距、(b) 對 T_team 接近 T_max 的解額外標示低信心/需要 GMAT 覆核、或 (c) 搜尋階段對這類候選解直接呼叫 GMAT 驗證。
2. **正式測資公布後，`configs/config.json` 整組要換成官方數字**（軌道六根數 + `k_t/C_t/k_v/C_v`）。現在的數字除了 `orbit_A.ECC=0` 是照簡報修正過的，其他都是我們自己編的，不能直接拿去用。
3. **固定 60 秒 RK4 步長**：這個已經跟第 1 項的關係釐清了——同一個極端軌道 A，單獨測 Python 內部 `dt=60s` vs `dt=0.1s` (不牽涉 GMAT)，整個 T_max 傳播只差 2.1 公里，遠遠不夠解釋第 1 項觀察到的 481 公里落差。**結論：步長本身大概率不是主要原因，缺失的 J3/J4 才是更可疑的方向**，自適應步長的優先順序可以往後放，先查 J3/J4 補上去能不能解釋大部分差距。
